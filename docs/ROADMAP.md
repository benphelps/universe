...
- [ ] Surface free camera: WASD + mouse-look movement on the ground. The ground-fixed frame is built for this — terrain vertices are static world geometry, up is radial, and the sky/frame contract (see frameQuat in unifiedViewer) keeps every sky object correct regardless of how the camera moves
