...
### Stars (with 02)
Photosphere shader: animated granulation (3D noise advected by differential rotation), limb darkening from the model coefficient, spot masks, chromaticity from the temperature table (uploaded LUT). Corona/prominences: layered fresnel rim + radial-noise billboards + curl-noise loop arcs. Compact exotics: white-dwarf point brilliance, pulsar beam cones sweeping with spin, black hole as a lensing post-pass (background distortion + photon-ring) with optional Doppler-beamed accretion disc.
