import { mu, seconds } from '../physics/units';
...
describe('elementsToState', () => {
  it('circular 1 AU orbit has Earth-like period and speed', () => {
    ...
    const { position, velocity } = elementsToState(el, MU_SUN, seconds(0));
  ...
  it('quarter period sweeps a quarter turn on a circular orbit', () => {
    const el = circularElements(AU);
    const period = orbitalPeriod(MU_SUN, AU);
    const p0 = elementsToState(el, MU_SUN, seconds(0)).position;
    const p1 = elementsToState(el, MU_SUN, seconds(period / 4)).position;
  ...
  it('eccentric orbit starts at periapsis and satisfies vis-viva everywhere', () => {
    ...
    const periapsis = elementsToState(el, MU_SUN, seconds(0)).position;
    expect(length(periapsis)).toBeCloseTo(2 * AU * 0.3, -6);

    const period = orbitalPeriod(MU_SUN, 2 * AU);
    for (let f = 0; f < 1; f += 0.09) {
      const { position, velocity } = elementsToState(el, MU_SUN, seconds(f * period));
  ...
  it('propagation is exact over many periods', () => {
    ...
    const a = elementsToState(el, MU_SUN, seconds(0.123 * period)).position;
    const b = elementsToState(el, MU_SUN, seconds((1000 + 0.123) * period)).position;
  ...
  it('inclination tilts the orbit out of plane', () => {
    ...
    const period = orbitalPeriod(MU_SUN, AU);
    const p = elementsToState(el, MU_SUN, seconds(period / 4)).position;
