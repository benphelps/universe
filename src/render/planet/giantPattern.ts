...
uniform vec2 uStormIdentity[${MAX_ACTIVE_STORMS}]; // texture seed, opacity
...
uniform vec2 uTemperature4; // mean, dipole in units of 1000 K^4
...
vec3 bandColorAt(float l, int band) {
  ...
  if (band + 1 < uBandCount) {
    ...
    float t = (1.0 - smoothstep(0.0, feather, uBands[band + 1].x - l));
    c = mix(c, fadedBandColor(band + 1), 0.5 * t);
  }
  if (band > 0) {
    float feather = mix(0.045, 0.016, uBands[band].w);
    float t = (1.0 - smoothstep(0.0, feather, l - uBands[band].x));
    c = mix(c, fadedBandColor(band - 1), 0.5 * t);
  }
  return c;
}
...
float stirField(float lat, float advLon, vec3 seed, vec3 churn) {
  vec3 e = vec3(cos(lat) * cos(advLon), sin(lat), cos(lat) * sin(advLon));
  float w1 = snoise(vec3(e.x, e.y * 2.2, e.z) * 1.7 + seed + churn * 0.3);
  float w2 = snoise(vec3(e.x, e.y * 3.4, e.z) * 4.4 - seed.yzx + churn.yzx * 0.6);
  float w3 = snoise(vec3(e.x, e.y * 5.0, e.z) * 11.0 + seed.zxy + churn.zxy * 0.9);
...
vec2 streakDeck(float wlat, float lonAdv, float streakHalf, vec3 seed, vec3 churn) {
  float deck = 0.0;
  float fine = 0.0;
  for (int j = 0; j < 4; j++) {
    float lo = lonAdv + (float(j) - 1.5) * streakHalf * 0.667;
    vec3 qq = vec3(cos(wlat) * cos(lo), sin(wlat), cos(wlat) * sin(lo));
    deck += fbm(vec3(qq.x, qq.y * 2.0, qq.z) * 3.0 + seed + churn);
    fine += fbm(vec3(qq.x, qq.y * 2.5, qq.z) * 9.0 + seed.yzx + churn.yzx * 1.6);
  }
...
void deckAt(in vec3 p, out vec3 surface, out float cloudH) {
  ...
  if (uRegime < 0.5) {
    ...
    float polarPhase = hemi > 0.0 ? uPolarPhase.x : uPolarPhase.y;
    float capEdge = polar.x;
    bool hexHere = polar.z > 0.5;
    if (hexHere) {
      capEdge += 0.05 * cos(polar.z * lon * hemi + polarPhase);
    }
    ...
    // Absolute cycle identities and bounded local ages arrive from the CPU.
    float wA = uWeatherTime.z;
    float wB = uWeatherTime.w;
    float ageA = uWeatherTime.x;
    float ageB = uWeatherTime.y;
    vec3 seedA = uSeedOffset + uWeatherSeedA;
    vec3 seedB = uSeedOffset + uWeatherSeedB;
    ...
    float warp = amp * wNorm * (wA * stirField(lat, lon + ageA * drift0, seedA, churn)
      + wB * stirField(lat, lon + ageB * drift0, seedB, churn));
    float wlat = clamp(lat + warp, -1.55, 1.55);
    int band = bandAt(wlat);
    vec3 bandColor = bandColorAt(wlat, band);
    // Height is independent of chromophore brightness.
    float bandMeanH = 0.5;
    cloudH = bandMeanH;
    ...
    vec2 df = wNorm * (wA * streakDeck(wlat, lonA, streakHalf, seedA, churn)
      + wB * streakDeck(wlat, lonB, streakHalf, seedB, churn));
    float deck = df.x;
    float fine = df.y;
    ...
    for (int i = 0; i < ${MAX_ACTIVE_STORMS}; i++) {
      ...
      vec4 s = uStorms[i];
      float stormSeed = uStormIdentity[i].x;
      float sz = abs(s.z);
      float elong = 1.3;
      vec3 stormColor;
      if (s.z < 0.0) {
      ...
      float fade = uStormIdentity[i].y * (s.z < 0.0 ? 1.0 - smooth01((s.w - 0.7) / 0.3) : 1.0);
      if (s.z < 0.0) {
        ...
        float plumeNoise = snoise(vec3(downstream * 3.3, local.y * 6.5, stormSeed * 4.1)
          + uSeedOffset.zxy + churn.zxy * 0.45);
        float plumeFine = snoise(vec3(downstream * 7.2, local.y * 13.0, stormSeed * 7.7)
          - uSeedOffset.yxz + churn.yzx * 0.8);
        ...
        float broadWarp = snoise(vec3(local * 0.82, stormSeed * 3.3)
          + uSeedOffset.zxy + churn * 0.14);
        float midWarp = snoise(vec3((local + localFlow * 0.25) * 1.9, stormSeed * 5.1)
          - uSeedOffset.yxz + churn.zxy * 0.25);
        float fine = snoise(vec3((local + localFlow * 0.12) * 4.2, stormSeed * 7.9)
          + uSeedOffset.xzy + churn.yzx * 0.42);
        ...
        float spiralPhase = -2.0 * sign(s.x + 1e-6) * theta
          ...
          + 2.35 * broadWarp + 0.95 * midWarp - uStormPhase;
        ...
        float wakeNoise = snoise(vec3(wakeX * 3.1, local.y * 7.0, stormSeed * 9.1)
          - uSeedOffset.zyx + churn.zxy * 0.55);
    ...
    if (capBlend > 0.01) {
      ...
      float capFray = snoise(vec3((adv + flow * 0.035) * 14.0, hemi * 5.1)
        - uSeedOffset.zxy + churn.zxy * 0.32);
    ...
    float localK = 1000.0 * pow(max(0.0, uTemperature4.x + uTemperature4.y * dot(p, uHotspotDirObj)), 0.25);
    // Condensates survive preferentially in the cooler transported hemisphere.
    float cloudiness = 1.0 - smoothstep(900.0, 1500.0, localK);
    float lonAdv = lon + uLockedPhase;
    vec3 q = vec3(cos(lat) * cos(lonAdv), sin(lat) * 3.5, cos(lat) * sin(lonAdv));
    float streaks = fbm(q * 2.6 + uSeedOffset + churn);
    surface = mix(uBandColors[0], uStormFresh, cloudiness * 0.55) * (1.0 + 0.3 * streaks);
...
export function createPatternUniforms(
  ...
  return {
    ...
    uStormCount: { value: 0 },
    uStormIdentity: { value: new Float32Array(MAX_ACTIVE_STORMS * 2) },
    ...
    uTemperature4: { value: [circulation.atmosphere.meanTemperature4 / 1e12, circulation.atmosphere.temperatureDipole4 / 1e12] },
  };
}
