...
void main() {
  ...
  vec3 a = posPc * uInvStretch;
  float dSq = dot(a, a);
  float reach = uRadiusPc * ${f(ENVELOPE_REACH)};
  if (dSq > reach * reach) { outCarve = 0.0; return; }
  float envelope = exp(${f(-ENVELOPE_TIGHTNESS)} * dSq / (uRadiusPc * uRadiusPc));
  vec3 q = a / uRadiusPc + vec3(uSeedOffset, 0.0, 0.0);
  float turbulence = ${f(TURBULENCE_MEAN)}
    ${TURBULENCE_OCTAVES.map(
      ([frequency, amplitude]) => `+ ${f(amplitude)} * shapeNoise(q * ${f(frequency)})`,
    ).join('\n    ')};
  float carved = envelope * (max(turbulence, 0.0) + ${f(TURBULENCE_LIFT)}) - ${f(CARVE_THRESHOLD)};
  outCarve = carved <= 0.0 ? 0.0 : pow(carved, ${f(CARVE_EXPONENT)});
}
`;
