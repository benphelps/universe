import { splitStellarLight } from '../../core/color/stellarLight';
...
export interface Neighborhood {
  ...
  colors: Float32Array;
  /** Combined optical Y power, with unit-Y colors, before attenuation. */
...
export function computeNeighborhood(
  ...
  for (const slot of starsNear(viewpoint, neighborRadiusPc(viewpoint))) {
    const source = unresolvedStarLight(slot.seed, slot.positionPc);
    const physical = source.primary;
    const light = splitStellarLight(source.rgb);
    if (light.luminosity <= 0) continue;
    ...
    luminosities.push(light.luminosity);
