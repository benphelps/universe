...
export function scenicWorlds(system: StarSystem, galaxy: string, filter: SceneFilter = 'all'): SceneCandidate[] {
  ...
  for (const [hostIndex, host] of hosts.entries()) for (const [planetIndex, planet] of host.planets.entries()) {
    ...
    if (rings && ringStrength > 0.08) {
      add('rings', planet.name, 0.5 + 0.4 * ringStrength + 0.1 * Number(!!planet.physical.appearance.banding),
        [`${rings.composition} rings · ${fmt(rings.outerPlanetRadii)} planet radii`, `${rings.gaps.length} ring gaps`],
      ...
      for (const [moonIndex, moon] of planet.moons.entries()) {
        ...
        if (diameter >= 3 && ringHeight >= 0.15 && clarity > 0.25 && moon.physical.bulk.radiusEarth > 0.03) {
          add('ringed-moon', moon.name, 0.25 * ringStrength + 0.3 * clamp(diameter / 15) + 0.25 * clarity + 0.2 * clamp(opening / 20),
            [`Parent ≈ ${fmt(diameter, 2)}° across at orbital radius`, `Up to ≈ ${fmt(opening, 2)}° ring opening · clear-sky potential`],
    ...
    for (const [index, body] of [planet, ...planet.moons].entries()) {
      ...
      if (solid && p.climate.hydrosphere === 'oceans' && !p.climate.snowball && coverage > 0.05 && coverage < 0.92 &&
          p.atmosphere.surfacePressureBar >= 0.03 && clarity > 0.25) {
        add('coastline', body.name, 0.45 * clarity + 0.35 * (1 - Math.abs(coverage - 0.5)) + 0.2 * clamp(p.appearance.clouds.coverage / 0.5),
          [`${Math.round(coverage * 100)}% water basins · land remains`, `${fmt(p.atmosphere.surfacePressureBar)} bar · ${Math.round(p.appearance.clouds.coverage * 100)}% cloud cover`],
          'Find a shoreline from orbit, descend, and frame a low sun. Exact shorelines and local cloud cover need visual scouting.', moon);
      }
      if (solid && p.climate.hydrosphere === 'magma' && coverage > 0 && clarity > 0.15) {
        add('lava', body.name, 0.45 * clarity + 0.35 * (coverage < 0.9 ? 1 : 0.3) + 0.2 * clamp(((p.climate.magmaTemperatureK ?? 1800) - 900) / 1000),
          [`${fmt(coverage * 100, 2)}% exposed melt`, coverage < 0.9 ? 'Dark terrain can contrast with lava' : 'Global magma surface'],
          'Approach the terminator from the night side; look for glowing melt against unlit relief.', moon);
      }
      if (solid && clarity > 0.35 && system.companions.length) {
        const other = hostIndex === 0 ? system.companions[0].star : system.star;
        const separation = system.companions[Math.max(0, hostIndex - 1)].elements.semiMajorAxis / AU;
        const orbit = planet.elements.semiMajorAxis / AU;
        // Orbital-scale proxy, not a solved simultaneous sunset or epoch geometry.
        const otherDistance = system.configuration === 'p-type' && hostIndex === 0 ? orbit : Math.max(separation, orbit);
        const hostLight = splitStellarLight(stellarBandRgb(host.star.luminosity, host.star.tEff)).luminosity / orbit ** 2;
        const otherLight = splitStellarLight(stellarBandRgb(other.luminosity, other.tEff)).luminosity / otherDistance ** 2;
        ...
        if (Math.min(hostLight, otherLight) > 1e-6 && ratio >= 0.01 && ratio <= 100 && angle > 0.5) add('binary', body.name,
          0.5 * clarity + 0.25 * (1 - Math.abs(Math.log10(ratio)) / 2) + 0.25 * clamp(Math.abs(host.star.tEff - other.tEff) / 4000),
          [`${host.star.spectralType} + ${other.spectralType}`, `Companion visible light ≈ ${fmt(ratio, 2)}× host at orbital scales`],
